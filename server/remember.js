...
export function isFundingRemembered(
  req: ExpressRequest,
  fundingSource: $Values<typeof FUNDING>,
  opts?: IsFundingRememberedOptions = getDefaultIsFundingRememberedOptions()
): boolean {
  const cookies = opts.cookies || req.cookies || {};
  const cookieSettings = COOKIE_SETTINGS[fundingSource] || {};

  if (
    cookieSettings.legacyRead &&
    cookieSettings.legacyKey &&
    cookies[cookieSettings.legacyKey]
  ) {
    return true;
  }
...
export function rememberFunding(
  req: ExpressRequest,
  res: ExpressResponse,
  fundingSources: $ReadOnlyArray<$Values<typeof FUNDING>>,
  opts?: RememberFundingOptions = {}
) {
  const { expiry } = opts;
  ...
  for (const fundingSource of fundingSources) {
    const fundingConfig = (funding[fundingSource] =
      sdkCookie.funding[fundingSource] || {});
...
type RememberFundingMiddleware = (
  ExpressRequest,
  ExpressResponse
) => void | ExpressResponse;

function parseFundingSources(
  commaSeparatedFundingSources
...
function setSecurityHeaders(
  req: ExpressRequest,
  res: ExpressResponse,
  { nonce, domain }: {| nonce: string, domain: string |}
) {
...
export function rememberFundingIframe({
  allowedClients = {},
}: RememberFundingIframeOptions): RememberFundingMiddleware {
  return (req, res) => {
    const {
    ...
    if (!commaSeparatedFundingSources) {
      return res
        .status(400)
        .send(`Expected ${QUERY_PARAM.FUNDING_SOURCES} query param`);
    }

    if (!sdkMeta) {
      return res
        .status(400)
        .send(`Expected ${QUERY_PARAM.SDK_META} query param`);
    }

    if (!clientID) {
      return res
        .status(400)
        .send(`Expected ${QUERY_PARAM.CLIENT_ID} query param`);
    }
    ...
    if (expiryTime && !expiryTime.match(/^\d+$/)) {
      return res
        .status(400)
        .send(`Expected ${QUERY_PARAM.EXPIRY} query param to be a number`);
    }
    ...
    res.status(200).send(`
            ...
            <head>
                ...
                ${getSDKLoader({ nonce })}
                <script nonce="${nonce}">
                    paypal.rememberFunding(${safeJSON(
                      fundingSources
                    )}, ${safeJSON({ expiry })});
                </script>
            </head>
