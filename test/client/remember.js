...
describe(`remember cases`, () => {
  ...
  it("should successfully remember a funding source", () => {
    ...
    if (query.domain !== getDomain()) {
      throw new Error(
        `Expected domain to be ${getDomain()}, got ${query.domain}`
      );
    }

    if (query.sdkMeta !== getSDKMeta()) {
      throw new Error(
        `Expected sdkMeta to be ${getSDKMeta()}, got ${query.sdkMeta}`
      );
    }

    if (query["client-id"] !== getClientID()) {
      throw new Error(
        `Expected client-id to be ${getClientID()}, got ${query["client-id"]}`
      );
    }

    if (query["funding-sources"] !== [FUNDING.VENMO].join(",")) {
      throw new Error(
        `Expected funding-sources to be ${[FUNDING.VENMO].join(",")}, got ${
  ...
  it("should remember multiple funding sources", () => {
    ...
    if (query["funding-sources"] !== [FUNDING.VENMO, FUNDING.ITAU].join(",")) {
      throw new Error(
        `Expected funding-sources to be ${[FUNDING.VENMO, FUNDING.ITAU].join(
          ","
        )}, got ${query["funding-sources"]}`
      );
    }
  });
