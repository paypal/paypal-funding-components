...
test("should successfully remember a funding source in the cookie using an iframe", () => {
  ...
  const middleware = rememberFundingIframe({
    allowedClients: {
      [clientID]: {
        allowedFunding: [FUNDING.VENMO],
        allowedDomains: [testDomain],
      },
    },
  });
  ...
  if (src !== sdkUrl) {
    throw new Error(
      `Expected to find script with src of ${sdkUrl}, got ${src}`
    );
  }
  ...
  if (!fundingSources) {
    throw new Error(
      `Expected rememberFunding to be called with fundingSources`
    );
  }
  ...
  if (fundingSources.length !== 1 || fundingSources[0] !== FUNDING.VENMO) {
    throw new Error(
      `Expected rememberFunding to be called with ${FUNDING.VENMO}`
    );
  }
...
test("should successfully remember multiple funding sources in the cookie using an iframe", () => {
  ...
  const middleware = rememberFundingIframe({
    allowedClients: {
      [clientID]: {
        allowedFunding: [FUNDING.VENMO, FUNDING.ITAU],
        allowedDomains: [testDomain],
      },
    },
  });
  ...
  if (
    ...
    fundingSources[0] !== FUNDING.VENMO ||
    fundingSources[1] !== FUNDING.ITAU
  ) {
    throw new Error(
      `Expected rememberFunding to be called with ${FUNDING.VENMO} and ${FUNDING.ITAU}`
    );
  }
...
test("should successfully remember a funding source in the cookie using an iframe with an expiry", () => {
  ...
  if (fundingSources.length !== 1 || fundingSources[0] !== FUNDING.ITAU) {
    throw new Error(
      `Expected rememberFunding to be called with ${FUNDING.ITAU}`
    );
  }

  if (expiry !== EXPIRY) {
    throw new Error(
      `Expected expiry to be ${EXPIRY}, got ${expiry || "undefined"}`
    );
  }
...
test("should give a 400 error if disallowed domain passed", () => {
  ...
  const middleware = rememberFundingIframe({
    allowedClients: {
      [clientID]: {
        allowedFunding: [FUNDING.VENMO],
        allowedDomains: ["https://www.zombo.com"],
      },
    },
  });
