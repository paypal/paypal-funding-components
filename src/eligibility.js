...
import { values, extendUrl } from "@krakenjs/belter/src";
import { getDomain } from "@krakenjs/cross-domain-utils/src";

import {
  REMEMBER_FUNDING_URI,
  SUPPORTED_FUNDING_SOURCES,
  REMEMBERABLE_FUNDING_SOURCES,
} from "./config";
...
function dropRememberFundingFrame(
  fundingSources: $ReadOnlyArray<$Values<typeof FUNDING>>,
  opts?: RememberedFundingFrameOpts = {}
) {
  const { expiry } = opts;
  ...
  frame.setAttribute(
    "src",
    extendUrl(`${getPayPalDomain()}${REMEMBER_FUNDING_URI}`, {
      query: {
...
export function rememberFunding(
  fundingSources: $ReadOnlyArray<$Values<typeof FUNDING>>,
  opts?: RememberFundingOpts = getDefaultRememberFundingOpts()
) {
...
export const getRememberedFunding = (): $ReadOnlyArray<
...
export const getRefinedFundingEligibility = (): FundingEligibilityType => {
...
export function isFundingEligible(
  ...
  return Boolean(
    fundingEligibility[fundingSource] &&
      fundingEligibility[fundingSource].eligible
  );
}

export function isFundingRecommended(
  fundingSource: $Values<typeof FUNDING>
): boolean {
  if (values(FUNDING).indexOf(fundingSource) === -1) {
    throw new Error(`Funding source ${fundingSource} is not supported`);
  ...
  return Boolean(
    fundingEligibility[fundingSource] &&
      fundingEligibility[fundingSource].recommended
  );
}
